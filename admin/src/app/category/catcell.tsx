      ...
      <div className='shadow-md rounded-md bg-white p-6'>
        <div className="flex flex-col">
          <div className="flex justify-between items-center mb-2">
            ...
            <button className="px-2 py-1 accentButton rounded-md text-sm" onClick={() => remove(name)}>Remove</button>
          </div>
            ...
            {subCategories.map(sub => (
              <div className='w-1/2 pr-2 mb-2' key={sub}>
                <div className='w-full flex text-gray-800 rounded-full py-1 px-3 border border-gray-400 rounded shadow justify-between'>
                  ...
                  <button className='w-fit' onClick={() => handleDelete(sub)}><FontAwesomeIcon icon={faCircleXmark} className='accentIcon'/></button>
                </div>
              </div>
            ))}
          </div>
